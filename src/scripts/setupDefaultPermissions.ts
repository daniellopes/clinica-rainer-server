  ...
  try {
    ...
    for (const role of roles) {
      for (const unidade of unidades) {
        console.log(`Configurando permissões para ${role} na unidade ${unidade}...`);
    ...
    console.log(`📋 Encontrados ${users.length} usuários para configurar permissões...`);

    for (const user of users) {
      console.log(`Configurando permissões para usuário: ${user.nome} (${user.role})`);
      ...
      if (!currentUser?.unidadesAcesso || currentUser.unidadesAcesso.length === 0) {
